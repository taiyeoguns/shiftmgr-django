...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--prod", action="store_true", help="Add data for production"
        )
    ...
    def handle(self, *args, **options):
        ...
        # set up static data and get roles
        _roles = self._set_up_static_data()
        ...
        for _ in range(options["num"]):
            ...
            dt = datetime.combine(shift.date, datetime.min.time()) + timedelta(
                hours=random.choice(range(8, 14)),
                minutes=random.choice([0, 15, 30, 45]),
            )

            start = timezone.make_aware(dt)
            end = start + timedelta(
                hours=random.choice(range(1, 3)), minutes=random.choice([0, 15, 30, 45])
            )
    ...
    def _create_super_user(self):
        ...
        Returns:
            User -- returns created admin user
        """
